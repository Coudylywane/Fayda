...
export class RequestApiService {
    ...
    try {
      ...
      let errorMessage = errorResponse.developerMessage || errorResponse.message || 'Erreur lors de l\'application de la décision';

      // Gérer l'erreur 404
      if (error.response?.status === 404 && errorMessage.includes('introuvable')) {
