...
import { CountrySelector2Component } from 'src/app/shared/components/country-selector2/country-selector2.component';
...
export class EditDahiraModalComponent implements OnInit {
  @Input() dahira!: Dahira;
  @Output() save = new EventEmitter<Partial<CreateDahira>>();
  ...
  ngOnInit(): void {
    ...
    this.dahiraForm.patchValue({
      dahiraName: this.dahira.dahiraName,
      email: this.dahira.email,
      phoneNumber: this.dahira.phoneNumber,
      country: this.dahira.location?.country,
      department: this.dahira.location?.department,
      region: this.dahira.location?.region,
      address: this.dahira.location?.address,
    });
  }
