...
export class UtilisateursPage implements OnInit, OnDestroy {
      ...
      map(([users, searchTerm]) => {
        ...
        if (this.activeFilter !== 'Tous') {
          filtered = filtered.filter((user) =>
            this.activeFilter === 'Actifs' ? user.active : !user.active
          );
        }
  ...
  setFilter(filterName: string) {
    ...
    this.loadUsers(); // Recharger avec le filtre actif
  }
  ...
  loadUsers() {
    this.store.dispatch(
      UsersActions.loadUsers({ page: this.currentPage, size: this.pageSize })
    );
  }
  ...
  onImageError(event: Event, user?: any) {
    ...
    if (target) {
      target.src = this.generateAvatarSVG(
  ...
  getColorForUser(user: any): string {
    ...
    const identifier =
      user?.firstName || user?.lastName || user?.email || 'default';
  ...
  //       this.store.dispatch(
  //         UsersActions.setSearchTerm({ searchTerm: data.searchTerm })
  //       );
  //     }
  //   }
  // }
  ...
  callUser(user: User) {
    if (user.phoneNumber) {
      ...
      const formattedPhone = cleanPhone.startsWith('221')
        ? cleanPhone
        : '221' + cleanPhone;
